...
export async function GET() {
  try {
    const pixels = await sql`
      SELECT id, name, platform, pixel_id, status, global, events
      FROM pixels
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { name, platform, pixel_id, status, global: isGlobal, events } = body

    const [pixel] = await sql`
      INSERT INTO pixels (name, platform, pixel_id, status, global, events)
      VALUES (
        ${name}, ${platform}, ${pixel_id},
        ${status ?? true}, ${isGlobal ?? false},
