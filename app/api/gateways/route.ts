...
export async function GET() {
  try {
    const gateways = await sql`
      SELECT id, name, status, methods, auth_type, field_defs, fields, docs, note
      FROM gateways
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { name, status, methods, auth_type, field_defs, fields, docs, note } = body

    const [gw] = await sql`
      INSERT INTO gateways (name, status, methods, auth_type, field_defs, fields, docs, note)
      VALUES (
        ${name},
        ${status ?? false},
